...
function field(props) {
  ...
  switch (fieldConfig.type) {
    ...
    case 'custom':
      element = (
        <div>
          <span className="color-white" style={{paddingLeft: '5px'}}> Measure to 1/4 inch.</span>
          {fieldConfig.config.map(item => (
            <div className="custom-div" key={item.id}>
              ...
              <span className="color-white">{unitChanged ? item.type : 'cm'}</span>
            </div>
          ))}
          <div style={{padding: '5px', textAlign:'right', color: '#9598A5'}}>
              cm
              <label class='switch'>
  ...
  return (
    <div>
      <div className="field-wrapper field-container">
        ...
        <span className='required color-white'>{fieldConfig.validation.required ? 'Required' : 'Optional'}</span>
        <div>{element}</div>
      </div>
